...
export default function Dashboard({ onNavigate }) {
    ...
    const StatCard = ({ icon, title, value, trend, trendValue, color }) => (
        <div className="bg-black/50 backdrop-blur-md rounded-2xl p-6 border border-amber-400/30 shadow-xl hover:shadow-2xl hover:border-amber-400/50 transition-all">
            <div className="flex items-start justify-between">
                <div className="flex-1">
                    ...
                    <h3 className={`text-4xl font-bold ${color}`}>{value.toLocaleString()}</h3>
                    {trend && (
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-300">Welcome back! Here's your inventory overview.</p>
                </div>
                <button
                    onClick={loadDashboardData}
                    className="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-black font-semibold rounded-lg transition-all shadow-lg"
                >
                    🔄 Refresh
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-black/50 backdrop-blur-md rounded-2xl p-6 border border-amber-400/30 shadow-2xl">
                    <h2 className="text-2xl font-bold text-white mb-6">📈 Stock Distribution</h2>
                ...
                <div className="bg-gradient-to-br from-amber-500/20 to-orange-500/20 backdrop-blur-md rounded-2xl p-6 border border-amber-500/30 shadow-2xl">
                    <div className="flex items-start gap-4">
                        <div className="text-4xl">⚠️</div>
                        <div className="flex-1">
                            <h3 className="text-2xl font-bold text-amber-300 mb-2">Stock Alert</h3>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-2xl">
                <h2 className="text-2xl font-bold text-white mb-6">🏆 Top Stock Items</h2>
                            ...
                            {stats.topProducts.length === 0 ? (
                                ...
                                stats.topProducts.map((product, index) => (
                                    <tr key={product.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="py-3 px-2 text-gray-300">{product.size} / {product.color}</td>
                                        <td className="py-3 px-2 font-mono text-sm text-gray-400">{product.barcode}</td>
                                        <td className="py-3 px-2 text-right font-semibold">₹{product.price_retail}</td>
                                        <td className="py-3 px-2 text-right">
                                            <span className={`px-3 py-1 rounded-full text-sm font-bold ${product.stock_quantity > 10 ? 'bg-green-500/20 text-green-400' :
